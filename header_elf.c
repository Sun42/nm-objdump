...
** Last update Sun Mar  1 23:16:09 2009 christophe sundas
*/
#include <string.h>
#include <sys/mman.h>
#include <fcntl.h>
...
int	check_type_elf(Elf32_Ehdr *elf, char *name)
{
  ...
  clss = get_elfclass(elf);
  machtyp = select_machine(elf->e_machine);
  ostyp = get_os(elf);
  if (printf("%s : file format %s-%s-%s\n", name, clss, machtyp, ostyp) == -1)
    my_putstr_error("printf error \n");
  if (printf("start address 0x%08x \n", (unsigned)elf->e_entry) == -1)
    my_putstr_error("printf error \n");
  return (0);
}
